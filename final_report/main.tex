...
\textbf{Importance.}
Hallucinations -- cases where a language model generates fluent but factually incorrect content -- represent one of the most critical reliability problems in deployed NLP systems. Detecting them automatically is essential for safe deployment in high-stakes domains such as medicine, law, and education. Rather than relying on external fact-checking, this project explores whether the model's own uncertainty (expressed via token-level log-probabilities) can serve as an intrinsic hallucination signal. In this report, that question is studied in a controlled HaluEval QA setting rather than claimed as a fully general solution for arbitrary LLM outputs.

\textbf{Dataset, Input, and Output.}
The project uses the \textbf{HaluEval QA} benchmark. Each original sample contains a question, a knowledge snippet, a factual answer, and a hallucinated answer. During preprocessing, each sample is expanded into two supervised rows, giving a balanced binary classification dataset. The system input is a (question, candidate answer) pair -- optionally augmented with the knowledge snippet as context -- and the output is a binary hallucination label. This setup is well suited to controlled benchmarking, but it may also introduce dataset-specific cues that are weaker in open-domain generations from modern LLMs.

\textbf{Technical Challenges.}
The task is technically challenging for several reasons. First, hallucinated answers are often fluent and superficially plausible, making surface-level detection difficult. Second, uncertainty signals derived from small local models (e.g., \texttt{distilgpt2}) may not transfer reliably to the larger models that produce hallucinations in practice, so a lightweight surrogate must be interpreted cautiously. Third, distinguishing genuine epistemic uncertainty from aleatoric uncertainty (inherent ambiguity in the question) is a non-trivial research problem. Finally, the binary construction of HaluEval introduces its own biases that must be understood during evaluation.
...
The literature review is ongoing, focusing heavily on recent advancements in hallucination detection and uncertainty quantification. The following thematic areas have been surveyed:

\textbf{Hallucination Taxonomy and Detection Frameworks.}
Huang et al. \cite{huang2025survey} provide a comprehensive taxonomy of hallucinations in modern LLMs, distinguishing between factuality issues and faithfulness issues. Their survey confirms the urgency of moving beyond simple text-overlap metrics toward intrinsic model evaluation, which directly motivates our use of probability distributions as a detection signal.

\textbf{Epistemic Uncertainty and Token Probabilities.}
Estimating predictive uncertainty in LLMs has recently shifted toward exploiting internal log-probabilities. Ling et al. \cite{ling2024uncertainty} formally separate aleatoric and epistemic uncertainty during in-context learning, validating that token-level probability variations effectively signal a model's lack of knowledge. We operationalize this finding in our entropy-based baseline. Furthermore, Das and Srihari \cite{das2024compos} demonstrated that multi-faceted LLM ensembles utilizing role-based prompting can robustly detect these hallucinations, inspiring the architecture of our proposed hybrid method.

\textbf{Real-World LLM Deployment and Reliability Risks.}
The necessity of our framework is underscored by recent attempts to deploy generative models in critical, domain-specific applications. For instance, Prado et al. \cite{prado2025enhancing} explore embedded LLM agents in aviation flight decks, Cao et al. \cite{cao2025toward} propose hybrid models for energy-safe industrial monitoring, and Abbas and Atwell \cite{abbas2025cognitive} utilize LLMs for automated student assessment. Across diverse domains—from education to Arabic language fine-tuning \cite{khader2025adapting}—the lack of reliable hallucination detection remains a primary bottleneck, justifying our focus on uncertainty-aware classification.
...
\begin{enumerate}
  ...
  \item \textbf{Entropy Classifier} (\texttt{entropy}). Token-level log-probabilities are extracted from a local causal LM (\texttt{distilgpt2}) running in \emph{memory mode} (no retrieved context). Aggregate features -- mean log-probability, total sequence entropy, and token count -- are passed to a logistic regression classifier. This baseline directly operationalises the epistemic-uncertainty hypothesis, but it should be read as a low-cost surrogate baseline rather than a strong approximation of modern chat-model factuality.
...
The preliminary metrics demonstrate that the pipeline is fully operational. The \textbf{Entropy Baseline} performs strongly (AUROC 0.970), showing that internal token probabilities contain useful signal for hallucination detection in HaluEval QA. At the same time, the \textbf{Lexical SVM} achieves an unusually high accuracy (94.3\%), which strongly suggests the presence of lexical artifacts or structural biases within the benchmark itself. This means the current results should be interpreted as evidence that the benchmark is learnable through multiple signal families, not as proof of broad generalisation to arbitrary LLM outputs. The \textbf{RAG Compare} method establishes a solid, functioning baseline (AUROC 0.791) following threshold inversion, confirming that relevant context improves the model's confidence for factual answers.
...
\textbf{Stronger LM backbone.} Replace \texttt{distilgpt2} with a larger instruction-tuned model (e.g., an open-weight instruction model) to test whether stronger generation quality actually translates into a better uncertainty surrogate. This comparison should be reported carefully, because stronger language modeling ability does not necessarily imply better factual calibration: teacher-forced likelihood often tracks fluency and stylistic plausibility more directly than factual correctness.

\textbf{Expanded uncertainty features.} Beyond mean log-probability and sequence entropy, add token-level variance, top-$k$ probability mass, and calibration metrics (Expected Calibration Error, Brier score) as features for the entropy classifier. Stronger uncertainty families such as semantic entropy and self-consistency across multiple sampled answers should also be prioritised, because they are better aligned with open-domain generation than a single small surrogate's token probabilities.
...
\textbf{Short-answer mitigation plan.}
If the final error analysis confirms that brief responses are the hardest subgroup, the detector will be extended with answer-length-aware handling. Concretely, this can include explicit answer-length features, separate thresholds for short and long answers, and stronger entity- or evidence-matching signals for short factual claims where token-level uncertainty is unstable.
...
\textit{A Survey on Hallucination in Large Language Models: Principles, Taxonomy, Challenges, and Open Questions}. 
ACM Computing Surveys.

\bibitem{ling2024uncertainty}
Ling, C., Zhao, X., Zhang, X., Cheng, W., Liu, Y., Sun, Y., ... \& Chen, H. (2024). 
\textit{Uncertainty Quantification for In-Context Learning of Large Language Models}. 
arXiv preprint arXiv:2402.10189. % Update with exact proceedings if published

\bibitem{das2024compos}
Das, S., \& Srihari, R. K. (2024). 
\textit{Compos Mentis at SemEval2024 Task6: A Multi-Faceted Role-based Large Language Model Ensemble to Detect Hallucination}. 
Proceedings of the 18th International Workshop on Semantic Evaluation (SemEval-2024).

\bibitem{prado2025enhancing}
Prado, J. P. D. A., Mangion, D. Z., Muscat, A., Mizzi, S., Manicolo, A., Zammit, B., \& Gauci, J. (2025). 
\textit{Enhancing Flight Deck Decision Support with Distributed GenAI: A Multi-Agent Approach}. 
2025 IEEE Aerospace Conference.

\bibitem{cao2025toward}
Cao, Q., Li, C., \& Shi, H. (2025). 
\textit{Toward Energy-Safe Industrial Monitoring: A Hybrid Language Model Framework for Video Captioning}. 
Applied Sciences, 15, 12848.

\bibitem{abbas2025cognitive}
Abbas, N., \& Atwell, E. (2025). 
\textit{Cognitive Computing with Large Language Models for Student Assessment Feedback}. 
Big Data and Cognitive Computing, 9, 112.

\bibitem{khader2025adapting}
Khader, K. A., Hussein, M. S., \& Abu-Issa, A. S. (2025). 
\textit{Adapting Large Language Models for Arabic: Comparative Evaluation, Fine-Tuning, and Ethical Deployment}. 
IEEE Access, 13, 182632-182650.
