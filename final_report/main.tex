...
\begin{table}[h!]
\centering
\caption{Preliminary validation results (small-scale run, $n_\text{val}=20$).}
\label{tab:results}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lccccl}
\toprule
\textbf{Model} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{Status} \\
\midrule
Lexical SVM    & --  & -- & -- & -- & Implemented; full-data run pending \\
Entropy        & --  & -- & -- & -- & Implemented; full-data run pending \\
RAG Compare    & --  & -- & -- & -- & Implemented; full-data run pending \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note:} Numerical metrics are not yet finalised because the small-scale run was used solely to validate pipeline correctness (data flow, feature extraction, and output serialisation). The \texttt{make\_report\_assets.py} script has been verified to aggregate metrics and generate plots into \texttt{reports/tables/} and \texttt{reports/figures/}. Full-dataset runs are scheduled as the immediate next step (see Section~\ref{sec:challenges}).

Despite the absence of consolidated numbers at this stage, the pipeline produces consistent predictions and metric JSON files for each baseline, confirming that the experimental infrastructure is sound.
...
The following visualisations are planned and partially scaffolded in \texttt{src/llm\_uncertainty/reporting.py}:

\begin{itemize}
  \item \textbf{Confusion matrices} for each baseline, to expose false-positive and false-negative patterns.
  \item \textbf{ROC and Precision-Recall curves} to compare the three baselines under varying decision thresholds, especially relevant for the RAG-compare method.
  \item \textbf{Calibration plots} (reliability diagrams) to assess whether predicted probabilities align with empirical accuracy, supporting the epistemic-uncertainty narrative.
  \item \textbf{Log-probability distribution plots} contrasting the score distributions for factual vs.\ hallucinated answers under memory and context modes.
  \item \textbf{Learning curves} to monitor overfitting and data efficiency of the entropy classifier as training size grows.
  \item \textbf{t-SNE or UMAP projections} of TF-IDF and log-probability feature spaces to visually inspect the separability of factual vs.\ hallucinated samples.
\end{itemize}

All figure outputs are saved to \texttt{reports/figures/} via \texttt{make\_report\_assets.py}.
...
\begin{itemize}
  ...
  \item \textbf{Commit history:} 8 commits as of the report date; consistent development documented.
...
|-- data/
|   |-- raw/                     # Reserved for raw downloads
|   |-- external/                # External resources placeholder
|   +-- processed/halueval_qa/   # Train / val / test JSONL splits
|-- docs/
|   |-- papers/                  # Literature review storage
|   +-- progress_report_template.md
|-- reports/
|   |-- figures/                 # Generated plots
|   +-- tables/                  # Metrics CSV (initial_results.csv)
|-- results/
|   |-- generations/             # Scored JSONL (memory & context)
|   |-- metrics/                 # Per-baseline metrics JSON
