...
We use the QA split of HaluEval~\cite{li2023halueval}. Each record provides a question, a knowledge snippet, one factual answer and one hallucinated answer. We expand each record into two supervised rows, factual ($y=0$) and hallucinated ($y=1$), producing balanced train/validation/test JSONL splits via \texttt{scripts/prepare\_dataset.py}. The main experiments use 15{,}999 training rows and 2{,}001 validation rows; the underlying validation split contains 2{,}002 rows, one of which is dropped during surrogate scoring and feature merging, so all reported validation metrics are computed on the same 2{,}001 scored rows. This paired construction is useful for controlled comparison, but it may also introduce dataset-specific cues.

\subsection{Local Language Model Scoring}

Token-level log-probabilities are extracted with \texttt{distilgpt2}~\cite{radford2019gpt2}, used as a cheap local surrogate. For each candidate answer we compute a set of log-probability summary features, including mean, sum, minimum and maximum token log-probability, perplexity, token count and variance and calibration summaries (expected calibration error and Brier score). Scoring is done in both \emph{memory mode} (question + answer) and \emph{context mode} (knowledge prepended), allowing us to measure whether external evidence changes model confidence.
...
Unlike the single-signal baselines in Section~\ref{sec:method}, which each use their own classifier, all four ablation settings share a single architecture---a linear SVM with an iteration cap---so that feature groups are compared under identical conditions. The \textit{uncertainty\_only} row is therefore not directly comparable to the Entropy Base row of Table~\ref{tab:main_results}, which uses a standardized logistic-regression classifier tuned for that feature set: the shared SVM's default decision threshold penalizes the low-dimensional uncertainty features, even though their ranking quality (AUROC) stays high.
